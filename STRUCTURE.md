...
├── models.py            # Data models (DepotItem, Chunks, Manifest)
...
class GalaxyDownloader:
    ...
    def download_item(self, item: DepotItem, ...) -> str:
    ...
    def _download_v1_blob(self, ...) -> str:
        """
    ...
    def _download_v2_item(self, ...) -> str:
        """
...
| `download_item(item, output_dir, ...)` | Download single item (auto-detects V1/V2) |
| `download_items_parallel(items, ...)` | Download multiple items in parallel |
| `_download_v1_blob(...)` | Internal: V1 range-based download |
| `_download_v2_item(...)` | Internal: V2 chunk-based download |
| `_download_range_chunk(task)` | Internal: Single range request |
| `_download_v2_chunk(chunk)` | Internal: Single V2 chunk download |
